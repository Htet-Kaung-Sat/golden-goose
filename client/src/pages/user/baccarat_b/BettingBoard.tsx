...
const BettingBoard: React.FC<GamePlayerProps> = ({ desk, game }) => {
  ...
  return (
    <>
      ...
      >
        <img src="/images/win.png" alt="win" className="w-[31rem] z-10" />
        <div className="absolute bottom-3 text-white text-5xl text-center font-bold bg-black/30 w-4/5 py-13">
          贏得 {Number(netAmount).toFixed(3)}
        </div>
      </div>
      <div
        className={cn(
          "absolute -top-[19.5rem] left-1/2 -translate-x-1/2 z-20 flex flex-col items-center font-bold pointer-events-none",
          netAmount <= -1 ? "opacity-100" : "opacity-0 pointer-events-none",
        )}
      >
        <img src="/images/lose.png" alt="lose" className="w-[31rem] z-20" />
        <div className="absolute bottom-6 text-white text-5xl text-center bg-black/30 w-4/5 py-13">
            ...
            (result: Result) =>
              result.baccarat_type === "B" && (
                ...
                >
                  ...
                  <span className="text-3xl mt-0.5 text-white">
                  ...
                  {(confirmedBets[result.key] || pendingBets[result.key]) && (
                    <div className="absolute top-1 right-1">
                      ...
                      <img
                        ...
                        alt="coin"
                        className="w-17 h-17 rounded-full rotate-swing"
                      />
                    </div>
                  )}
                </BettingBoardItemWrapper>
      ...
      <div className="flex items-center justify-center gap-5 absolute right-0">
        <div
          className={cn(
            "flex bg-black py-3 px-5 items-center gap-2 rounded-md",
