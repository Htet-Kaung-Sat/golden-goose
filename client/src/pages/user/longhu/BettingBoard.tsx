...
const BettingBoard: React.FC<GamePlayerProps> = ({ desk, game }) => {
  ...
  // useRoundResult hook to handle the round result and net amount
  ...
  const handleBetClick = (result_id: number, key: string) => {
    ...
    // calculate final user balance after selected coin
  ...
  // get pending bets from local storage while status is betting
  useEffect(() => {
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-[23rem] left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none",
          netAmount >= 1 ? "opacity-100" : "opacity-0 pointer-events-none",
        )}
      >
        <img src="/images/win.png" alt="win" className="h-[35rem] z-10" />
        <div className="absolute bottom-10 text-white text-5xl text-center font-bold bg-black/40 w-4/5 py-14">
          贏得 {Number(netAmount).toFixed(3)}
        </div>
      </div>
      <div
        className={cn(
          "absolute -top-[22rem] left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none",
          netAmount <= -1 ? "opacity-100" : "opacity-0 pointer-events-none",
        )}
      >
        <img src="/images/lose.png" alt="lose" className="h-[35rem] z-20" />
        <div className="absolute bottom-12 text-white text-5xl text-center font-bold bg-black/40 w-4/5 py-14">
      ...
      <div className="flex justify-center gap-3 p-1 w-full mb-8">
        <div className="flex justify-center items-stretch bg-[#0b2c0b]/80 border-3 border-gray-300 rounded-lg overflow-hidden">
          {game.results.map((result: Result) => (
              ...
              {(confirmedBets[result.key] || pendingBets[result.key]) && (
                <div className="absolute top-1 right-1">
                  ...
                  <img
                    ...
                    alt="coin"
                    className="w-20 h-20 rounded-full rotate-swing"
                  />
                </div>
              )}
            </BettingBoardItemWrapper>
      ...
      <div className="flex items-center justify-center gap-5 absolute right-0">
        <div
          className={cn(
            "flex justify-center items-center gap-2 bg-black py-3 px-5 rounded-md",
