...
/**
 * Baccarat G betting board. Uses shared useRoundResult (socket + poll) so result
 * and winner state apply even when socket misses updateResult; onResult clears
 * pending/confirmed bets and resets balance/amount, same as longhu BettingBoard.
 */
const BettingBoard: React.FC<GamePlayerProps> = ({ desk, game }) => {
  ...
  const handleBetClick = (result_id: number, key: string) => {
    ...
    if (isBetRestricted(desk.last_round?.round_no, key)) {
  ...
  const firstColumn = game.results.filter(
    (r: Result) =>
      r.baccarat_type === "G" && (r.key === "big" || r.key === "anyPair"),
  );

  const lastColumn = game.results.filter(
    (r: Result) =>
      r.baccarat_type === "G" && (r.key === "small" || r.key === "perfectPair"),
  );

  const middleColumns = game.results.filter(
    (r: Result) =>
      r.baccarat_type === "G" &&
      !["big", "anyPair", "small", "perfectPair"].includes(r.key),
  );
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-[20.5rem] left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none",
          netAmount >= 1 ? "opacity-100" : "opacity-0 pointer-events-none",
        )}
      >
        <img src="/images/win.png" alt="win" className="h-[31rem] z-10" />
        <div className="absolute bottom-3 text-white text-5xl text-center font-bold bg-black/40 w-4/5 py-13">
          贏得 {Number(netAmount).toFixed(3)}
        </div>
      </div>
      <div
        className={cn(
          "absolute -top-[19.5rem] left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none",
          netAmount <= -1 ? "opacity-100" : "opacity-0 pointer-events-none",
        )}
      >
        <img src="/images/lose.png" alt="lose" className="h-[31rem] z-20" />
        <div className="absolute bottom-7 text-white text-5xl text-center font-bold bg-black/40 w-4/5 py-13">
      ...
      <div className="flex justify-center gap-3 p-1 w-full mb-8">
        <div className="flex justify-center items-stretch bg-[#0b2c0b]/80 border-3 border-gray-300 rounded-lg overflow-hidden">
          <div className="flex flex-col w-[180px] h-[150px]">
            <div className="flex flex-col w-[180px] h-[150px]">
              {firstColumn.map((result, index) => (
                <BetCell
                  key={`${result.key}-${changeResult}-${index}`}
                  result={result}
                  half
                  onClick={() => {
                    if (result.id) {
                      handleBetClick(result.id, result.key);
                    }
                  }}
          ...
          {middleColumns.map((result, index) => (
            <BetCell
              key={`${result.key}-${changeResult}-${index}`}
              result={result}
              onClick={() => {
      ...
      <div className="flex items-center justify-center gap-5 absolute right-0">
        <div
          className={cn(
            "flex items-center bg-black py-3 px-5 gap-2 rounded-md",
            (hasConfirmedBets || hasPendingBets) && status === "betting"
              ? "opacity-100"
              : "opacity-0",
          )}
        >
    ...
    <BettingBoardItemWrapper
      ...
      className={
        half
          ...
          : "w-[180px] h-[150px] border-l-3 border-gray-300"
      }
    >
      <span
        className={cn(
          "font-bold",
          half ? "text-3xl" : "text-6xl",
