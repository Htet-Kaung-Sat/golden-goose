...
const ResultTable: React.FC<ResultTableProps> = ({
  ...
  const bankerCount =
    results?.filter((r) => r.key.split("|").includes("banker") && !r.isGuess)
      .length || 0;
  const playerCount =
    results?.filter((r) => r.key.split("|").includes("player") && !r.isGuess)
      .length || 0;
  const tieCount =
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-row text-white text-center p-1 font-bold w-full">
        <div className="flex items-center gap-2 justify-between px-4 border-none">
          <span className="text-red-500">庄</span>
          <span className="text-white">{bankerCount}</span>
        </div>
        <div className="flex items-center gap-2 justify-between px-4 border-none">
          <span className="text-[#01fae6]">闲</span>
          <span className="text-white">{playerCount}</span>
        </div>
        <div className="flex items-center gap-2 justify-between px-4 border-none">
          <span className="text-green-500">和</span>
          <span className="text-white">{tieCount}</span>
        </div>
