...
const BettingBoard: React.FC<BettingBoardProps> = ({
  ...
  const get = (key: string) => game.results.find((r) => r.key === key);
  ...
  return (
    <>
      ...
      <div className="flex justify-center gap-3 p-1 w-full">
        <div className="bg-[#0b2c0b]/80 border-3 border-gray-300 overflow-hidden w-full">
          <div className="grid grid-cols-3">
            {[get("dragon"), get("tie"), get("tiger")].map(
              (r) =>
                r && (
                  <BetCell
          ...
          <div className="grid grid-cols-2 border-t-3 border-gray-300">
            {[get("dragonDouble"), get("tigerDouble")].map(
              (r) =>
                r && (
                  <BetCell
                    key={r.key + changeResult}
          ...
          <div className="grid grid-cols-2 border-t-3 border-gray-300">
            {[get("dragonSingle"), get("tigerSingle")].map(
              (r) =>
                r && (
                  <BetCell
                    key={r.key + changeResult}
...
const BetCell: React.FC<BetCellProps> = ({
  ...
  const total = pendingAmount + confirmedAmount;
  ...
  return (
    <BettingBoardItemWrapper
      ...
      className={cn(
        "w-full  border-l-3 first:border-l-0 border-gray-300  ",
        large && "h-[110px]",
        small && "h-[70px]",
      )}
    >
      ...
      <span className={cn("font-bold", large ? "text-6xl" : "text-3xl", color)}>
        {result.name}
      </span>
