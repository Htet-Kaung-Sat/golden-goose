...
const Index: React.FC<MultipleBetProps> = ({
  ...
  const { multipleDesks, setMultipleDesks, amount, setAmount, selectedCoin, setSelectedCoin } = useGameContext();
  ...
  return (
    <div className="relative w-full h-full bg-[#1e1611] overflow-hidden">
      <div
        ref={scrollRef}
        onWheel={(e) => {
          if (!scrollRef.current) return;
      ...
      >
        {multipleDesks.map((desk) => {
          ...
          return (
            <div
              key={desk.id}
              className="relative w-[385px] h-[780px] flex-shrink-0 bg-[#3a2a1b] rounded-lg border border-yellow-600/30 overflow-hidden"
            >
              <div
                className={cn(
                  "absolute top-0 left-0 right-0 z-20 h-14 px-4",
                  "flex items-center justify-between",
                  "bg-gradient-to-b from-[#4b3524] to-[#2a1c12]",
                  ...
                  <>
                    ...
                    {deskCameras.length > 1 && (
                      <button
                        ...
                        className="absolute bottom-5 right-5 z-30 bg-[#2c1f16] text-white p-1 rounded-sm hover:scale-110 transition"
                      >
                        <Icon
        ...
        <div className="relative w-[385px] h-[780px] flex-shrink-0 bg-[#3a2a1b] rounded-lg border border-yellow-600/30 overflow-hidden">
          <div
            onClick={() => setShowDeskPanel(true)}
            className={cn(
              "h-[780px] flex flex-col items-center justify-center",
              "bg-gradient-to-b from-[#3a2a1b] to-[#2a1c12]",
              "border border-yellow-600/30 rounded-lg cursor-pointer",
              "hover:shadow-[0_0_20px_rgba(210,155,36,0.25)]",
            )}
          >
            <div className="w-20 h-20 rounded-full border-2 border-yellow-500 flex items-center justify-center mb-4">
              <Icon icon="mdi:plus" className="text-yellow-400" width={48} />
            </div>
            <span className="text-3xl text-white tracking-wide">添加桌台</span>
          </div>
        </div>
      </div>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-yellow-600/30">
        ...
        {/* Footer Filter */}
        <div className="flex gap-3 p-4 border-t border-yellow-600/30 bg-[#2a1c12]">
          <Button
